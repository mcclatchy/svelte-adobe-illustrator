...
var defaultSettings = {
  ...
  "svelte_output_path": "/ai2svelte-output/",
  "svelte_output_extension": ".svelte",
  "svelte_image_output_path": "/imgs/",
  ...
  "settings_block": [
    ...
    "svelte_output_path",
    "svelte_output_extension",
    "svelte_image_output_path",
    "image_output_path",
    "local_preview_template",
...
function initDocumentSettings(textBlockSettings) {
  ...
  // Auto apply mcclatchy settings
  extendSettings(settings, mcclatchyOverrideSettings);
...
function convertArtItems(activeArtboard, textFrames, masks, settings) {
  ...
  checkForOutputFolder(getImageFolder(settings), 'svelte_image_output_path');
...
function getImageFolder(settings) {
  return pathJoin(docPath, settings.svelte_output_path, settings.svelte_image_output_path);
}
...
function generateImageHtml(imgFile, imgId, imgClass, imgStyle, ab, settings) {
  var imgDir = "{ imagePath.replace(new RegExp('/([^/.]+)$'), '/$1/') || '/' }" + settings.image_source_path,
      imgAlt = encodeHtmlEntities(settings.image_alt_text || ""),
      html,
      src;
  if (imgDir === null) {
    imgDir = settings.svelte_image_output_path;
  }
...
function getResizerScript(containerId, settings) {
  ...
  svelteJS += "export let imagePath = './';\r";
...
function generateOutputHtml(content, pageName, settings) {
  ...
  htmlFileDestinationFolder = docPath + settings.svelte_output_path;
  checkForOutputFolder(htmlFileDestinationFolder, 'svelte_output_path');
  htmlFileDestination = htmlFileDestinationFolder + pageName + settings.svelte_output_extension;

  if (settings.output == 'one-file' && settings.project_type == 'ai2html') {
    htmlFileDestination = htmlFileDestinationFolder + 'index' + settings.svelte_output_extension;
  }
